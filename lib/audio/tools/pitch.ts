...
export async function runPitch(
  ...
  const output = await ffmpegExec(wavBlob, 'input.wav', 'output.mp3', [
    '-af', `asetrate=${newRate},aresample=${buffer.sampleRate}`,
  ...
  return { blob, name: file.name.replace(/.[^.]+$/, `-pitch${sign}${semitones}st.mp3`), size: blob.size, mimeType: 'audio/mpeg', format: 'MP3' }
}
