...
export async function runJoin(
  ...
  if (crossfade > 0 && files.length === 2) {
    ...
    const _filterChain = `${files.map((_, i) => `[a${i}]`).join('')}concat=n=${files.length}:v=0:a=1[out]`
    const _filterInputs = files.map((_, i) => `[a${i}]`).join('')
    const amix = files.map((_, i) => `[${i}:a]`).join('')
    await ffmpeg.exec([
      ...inputs,
      '-filter_complex',
      `${amix}concat=n=${files.length}:v=0:a=1[outa]`,
      '-map',
      '[outa]',
      '-c:a',
      'libmp3lame',
