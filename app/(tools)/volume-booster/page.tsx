...
export default function VolumeBoosterPage() {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      setResult(res); toast.success('Done!')
    } catch { setError('PROCESS_FAILED') }
    finally { setProgress(null); endProcessing() }
  }

  return (
    <ToolShell tool={tool} description="Adjusts volume using Web Audio API GainNode with a soft limiter to prevent clipping.">
      ...
      {file && !progress && !result && (
        <div className="flex flex-col gap-5">
          ...
          <div>
            <p className="text-sm font-medium mb-2 text-[#1A1208] dark:text-[#FFF8ED]">
              Gain: <span className="font-mono">{gainDb >= 0 ? '+' : ''}{gainDb} dB</span>
            </p>
            <input type="range" min={-20} max={20} step={1} value={gainDb} onChange={(e) => setGainDb(Number(e.target.value))} className="w-full accent-orange-500" aria-label="Volume gain" />
          </div>
