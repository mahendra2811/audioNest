...
export default function AudioReverserPage() {
  ...
  return (
    <ToolShell
      tool={tool}
      description="Reverses all audio channels on-device using the Web Audio API OfflineAudioContext."
    >
      {!file && !progress && (
        <Dropzone onFile={handleFile} accept={ACCEPT} label="Drop audio to reverse" />
      )}
      {progress && (
