...
export default function StereoToMonoPage() {
  ...
  const handleFile = async (f: File) => {
    ...
    try {
      ...
      setResult(res); toast.success('Converted to mono!')
    } catch { setError('PROCESS_FAILED') }
    finally { setProgress(null); endProcessing() }
  }

  return (
    <ToolShell tool={tool} description="Downmixes stereo to mono using ffmpeg -ac 1 on-device. Halves the file size.">
      {!file && !progress && <Dropzone onFile={handleFile} accept={ACCEPT} label="Drop stereo audio" />}
